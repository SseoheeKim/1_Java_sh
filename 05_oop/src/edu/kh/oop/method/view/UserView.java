...
// 화면의 입/출력이 구현될 클래스
	...
	// UserView 클래스 내에서 계속 사용할 수 있는 Scanner생성
	
	private UserService service = new UserService(); // UserService 객체 생성
	private User user = null; // User참조 변수 선언(아직 참조하는 객체 없음) 
							// 회원 가입 정보를 담고 있는 User객체
	
	private User loginUser = null;; // 로그인한 User 객체의 주소를 참조할 변수
	//null인 경우 == 로그인 X
	//null이 아닌 경우 == 로그인 O
	
	
	public void displayMenu() {
		int input = 0; // 메뉴 입력용 변수, 메뉴 종료
		...
		do {
			...
			switch(input) {
			case 1 : this.signUp(); break; //this. 는 생략가능
		...
		} while(input != 0); // 메뉴 0번이 아닌 선택을 할 경우 반복
	}
	
	// 1. 회원 가입 정보를 입력받을 메서드(화면에 구현되어야 하니까 View에 메서드 생성)
	public void signUp() {
		...
		// 회원 가입 서비스 호출
		user = service.signUp(id, pw, name, age, gender); // User객체의 주소 반환
		// 필드에 존재하는 user 참조 변수에 반환된 User객체의 주소를 저장
	...
	public void login(){
		...
		if(loginUser != null) { 
			...
			if(result == -1) {
				...
				loginUser = user; 
				// user변수가 참조하는 객체의 주소를 loginUser에 대입(얕은 복사)
			}
		}
	}
	...
	// 3.로그아웃 화면
	public void logout() {
		if(loginUser != null) { // 로그인이 되어있는 경우
			loginUser = null; // 로그인유저를 null로 바꾸면 로그아웃
